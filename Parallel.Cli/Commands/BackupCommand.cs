...
namespace Parallel.Cli.Commands
{
    public class BackupCommand : Command
    {
        ...
        public BackupCommand() : base("backup", "Backs up the system to vaults.")
        {
        ...
        private async Task BackupSystemAsync(LocalVaultConfig vault, bool force)
        {
            ...
            try
            {
                ...
                {
                    await BackupPathInternalAsync(syncManager, path, force);
                }
            }
            finally
            {
        ...
        private async Task BackupPathInternalAsync(ISyncManager syncManager, string path, bool force)
        {
            ...
            _totalFiles += await syncManager.PushFilesAsync(files, new ProgressReport(syncManager.RemoteVault, successFiles), force);
            CommandLine.WriteLine(syncManager.RemoteVault, $"Successfully pushed {_totalFiles:N0} files in {_sw.Elapsed}.", ConsoleColor.Green);
        }
