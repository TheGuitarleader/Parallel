...
namespace Parallel.Core.Database.Contexts
{
    ...
    public class SqliteContext : IDatabase
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            await _semaphore.ExecuteAsync("CREATE TABLE IF NOT EXISTS `objects` (`path` TEXT NOT NULL, `hash` TEXT NOT NULL, orderIndex INTEGER NOT NULL, UNIQUE (path, orderIndex));");
            await _semaphore.ExecuteAsync("CREATE TABLE IF NOT EXISTS `files` (`name` TEXT NOT NULL, `localpath` TEXT NOT NULL, `remotepath` TEXT NOT NULL, `lastwrite` LONG INTEGER NOT NULL, `lastupdate` LONG INTEGER NOT NULL, `localsize` LONG INTEGER NOT NULL, `remotesize` LONG INTEGER NOT NULL, `type` TEXT NOT NULL DEFAULT Other CHECK(`type` IN ('Document', 'Photo', 'Music', 'Video', 'Other')), `hidden` INTEGER NOT NULL DEFAULT 0, `readonly` INTEGER NOT NULL DEFAULT 0, `deleted` INTEGER NOT NULL DEFAULT 0, `checksum` TEXT, UNIQUE (localpath, checksum));");
